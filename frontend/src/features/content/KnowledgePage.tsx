...
function badgeLabel(kind: ContentKind, value: string | number | null) {
  if (kind === 'wiki') return value === 'business' ? 'Nghiệp vụ' : value === 'technical' ? 'Kỹ thuật' : value || '—';
...
export default function KnowledgePage({ kind }: { kind: ContentKind }) {
  ...
  async function load() {
    if (!legacyMode) { setError('Target chưa có API Wiki/Thông báo được phê duyệt.'); return; }
    setLoading(true);
    setError(null);
    try {
      if (wiki) {
        setWikiItems(await legacyAuthClient.wikiList({ category, product, business_area: businessArea, client, level: levelOrPriority, search }));
      } else {
  ...
  }, [businessArea, category, client, kind, legacyMode, levelOrPriority, product, search]);

  const items = wiki ? wikiItems : announcementItems;
  const products = [...new Set(wikiItems.map(item => item.product).filter((value): value is string => Boolean(value)))].sort();
  const businessAreas = [...new Set(wikiItems.map(item => item.business_area).filter((value): value is string => Boolean(value)))].sort();
  const clients = [...new Set(wikiItems.map(item => item.client).filter((value): value is string => Boolean(value)))].sort();
  const title = wiki ? 'Wiki nội bộ' : 'Thông báo & Tài liệu';

  return <section className="knowledge-page">
    <div className="page-header-wrapper">
      <div className="page-title-row"><h1 className="page-title"><span className="title-icon knowledge-title-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d={wiki ? 'M4 19.5A2.5 2.5 0 0 1 6.5 17H20' : 'M21 3L9 9H3a1 1 0 0 0-1 1v4a1 1 0 0 0 1 1h2l2 7h2l12 6V3z'} /><path d={wiki ? 'M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z' : 'M15.54 8.46a5 5 0 0 1 0 7.07'} /></svg></span>{title}</h1></div>
      <div className="page-toolbar knowledge-toolbar">
        <div className="toolbar-filters">
          <select value={category} onChange={event => setCategory(event.target.value)}><option value="">-- Phân loại --</option>{wiki ? <><option value="business">Nghiệp vụ</option><option value="technical">Kỹ thuật</option></> : <><option value="announcement">Thông báo</option><option value="regulation">Nội quy</option><option value="document">Tài liệu</option><option value="guide">Hướng dẫn</option></>}</select>
          {wiki && <><select value={product} onChange={event => setProduct(event.target.value)}><option value="">-- Sản phẩm --</option>{products.map(value => <option key={value} value={value}>{value}</option>)}</select><select value={businessArea} onChange={event => setBusinessArea(event.target.value)}><option value="">-- Nghiệp vụ --</option>{businessAreas.map(value => <option key={value} value={value}>{value}</option>)}</select><select value={client} onChange={event => setClient(event.target.value)}><option value="">-- CTCK --</option>{clients.map(value => <option key={value} value={value}>{value}</option>)}</select></>}
          <select value={levelOrPriority} onChange={event => setLevelOrPriority(event.target.value)}><option value="">-- Mức độ --</option>{wiki ? <><option value="Immediate">Immediate</option><option value="High">High</option><option value="Normal">Normal</option><option value="Low">Low</option></> : <><option value="0">Thường</option><option value="1">Quan trọng</option><option value="2">Khẩn cấp</option></>}</select>
          <input value={search} onChange={event => setSearch(event.target.value)} placeholder="Tìm kiếm toàn văn..." />
        </div>
        <div className="toolbar-actions"><button type="button" className="btn btn-tools" onClick={() => void load()} disabled={loading}>Làm mới</button><button type="button" className="btn btn-success" disabled title="Chưa có target API và permission CRUD.">Thêm mới</button></div>
      </div>
    </div>
